...
<section class="article theme-dark">
  <div class="container">
    <article>
      <nav class="back">
        <a href="<?= $page->parent()->url() ?>">&larr; Terug naar jobs</a>
      </nav>
      ...
      <div class="text">
        ...
        <?php if ($page->text()->value()): // check if exists ?>
          ...
          <section class="job-form">
              <header>
                <h2>Solliciatie</h2>
                <p><?= $page->formRequired()->html() ?></p>
              </header>

              <form action="<?= $page->url() ?>" method="post">
              <div>
                  <label for="name"><?= page('jobs')->labelName()->html() ?></label> <span>*</span>
                ...
                <div class="honeypot">
                  <label><?= page('jobs')->formHoneypot()->html() ?></label> <span>*</span>
                  <?php echo honeypot_field('website', 'field'); ?>
                </div>
                <div>
                  <label for="surname"><?= page('jobs')->labelSurName()->html() ?></label> <span>*</span>
                  <input<?php if ($form->error('name')): ?> class="error"<?php endif; ?> name="name" type="text" value="<?= $form->old('surname') ?>">
                  <?php snippet('form/error', ['field' => 'surname']) ?>
                </div>
                <div>
                  <label for="email"><?= page('jobs')->labelEmail()->html() ?></label> <span>*</span>
                ...
                <div>
                  <label for="message"><?= page('jobs')->labelMessage()->html() ?></label> <span>*</span>
                ...
                <div>
                  <?= page('jobs')->notice()->html() ?>
                </div>
                <div>
                  <?php echo csrf_field() ?>
                  <input type="submit" value="<?= page('jobs')->nextButton()->html() ?>" class="btn">
        ...
        <?php else: // fallback ?>
          <blockquote class="warning">Deze pagina heeft nog geen content.</blockquote>
        <?php endif ?>
      </div>
